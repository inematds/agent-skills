...
(function (window, document) {
  ...
  function renderHighlights(container) {
    ...
    for (var blockId in notes) {
      ...
      if (!block) continue; // Other course pages own their blocks.
  ...
  function navigateToCourseAnchor(anchor, blockId) {
  ...
  function saveCheckpoint(topicId) {
    if (!topicId && !topicEls().length) return;
  ...
  function buildExportObject() {
    return Object.assign({}, storageGet(key('extra'), {}), {
      schemaVersion: SCHEMA_VERSION,
      courseId: S.courseId,
  ...
  function importJSON(text, opts) {
    ...
    if (!valid) { res.errors.push("Archivo incompatible: curso, versión o estructura no válidos."); return res; }
